...
import com.alibaba.csp.sentinel.dashboard.api.gateway.DynamicGatewayApiDefinitionProvider;
import com.alibaba.csp.sentinel.dashboard.api.gateway.DynamicGatewayApiDefinitionPublisher;
...
public class GatewayApiControllerV2 {
    ...
    @Autowired
    @Qualifier("gatewayApiDefinitionApolloProvider")
    private DynamicGatewayApiDefinitionProvider<List<ApiDefinitionEntity>> apiDefinitionProvider;
    @Autowired
    @Qualifier("gatewayApiDefinitionApolloPublisher")
    private DynamicGatewayApiDefinitionPublisher<List<ApiDefinitionEntity>> apiDefinitionPublisher;

    @GetMapping("/list.json")
    public Result<List<ApiDefinitionEntity>> queryApis(HttpServletRequest request, String app, String ip, String port) {
        ...
        try {
            List<ApiDefinitionEntity> apis = apiDefinitionProvider.getApis(app, ip, port);
    ...
    private boolean publishApis(String app, String ip, Integer port) {
        ...
        try {
            apiDefinitionPublisher.publish(app, ip, String.valueOf(port), apis);
            return true;
        } catch (Exception e) {
            logger.error("发布API的接口失败 :", e);
            return false;
        }
    }
}
